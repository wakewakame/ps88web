...
const shapes = {
  ...
  smooth_rect: (cx, cy, w, h, r1, r2, div) => {
    ...
    return [
      ...corner.map((p) =>
        vec2.add(vec2.rot90(p, 0), [cx - w_half, cy - h_half]),
      ),
      ...corner.map((p) =>
        vec2.add(vec2.rot90(p, 1), [cx + w_half, cy - h_half]),
      ),
      ...corner.map((p) =>
        vec2.add(vec2.rot90(p, 2), [cx + w_half, cy + h_half]),
      ),
      ...corner.map((p) =>
        vec2.add(vec2.rot90(p, 3), [cx - w_half, cy + h_half]),
      ),
    ];
  },
  ...
  circle: (r, div) => {
    ...
    return [...Array(div)].map((_, i) => [
      r * Math.cos((2 * Math.PI * i) / div),
      r * Math.sin((2 * Math.PI * i) / div),
    ]);
  },
