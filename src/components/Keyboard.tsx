  ...
  return (
    <div className="w-full h-full flex flex-row items-end justify-center">
      {/* オクターブを下げるボタン */}
      {
        (octave > -1) ?
          <button
            onClick={() => { setOctave(Math.max(octave - 1, -1)); }}
      ...
      {[...Array(25)].map((_, i) => {
        ...
        return (0 <= note && note <= 127) ?
          <button
            ...
            onPointerEnter={(e) => { if (e.buttons === 1) keyDown(note); }}
            onPointerLeave={(e) => { if (e.buttons === 1) keyUp(note); }}
            className={`
            ...
            { note % 12 == 0 ? ( <span className="text-zinc-500">{`C${(note-12)/12}`}</span>) : null }
          </button> :
      ...
      {/* オクターブを上げるボタン */}
      {
        (octave < 8) ?
          <button
            onClick={() => { setOctave(Math.min(octave + 1, 8)); }}
            className="group relative flex-1 h-4/5 cursor-pointer"
          >
            <span className="
